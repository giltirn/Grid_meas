...
int main(int argc, char** argv){
...

  {
    ...
    sliceSum(test, C, 3);
    std::cout << GridLogMessage << "X(t) = \\sum_\\vec x tr(R(x)R^dag(x)) for prop with t0=" << 1 << std::endl;
    for(int t=0;t<Lt;t++){
...

  {
    ...
    std::vector<SCFmatrixD> C;
    sliceSum(test, C, 3);
    std::cout << GridLogMessage << "X(t) = \\sum_\\vec x tr(R(x)R^dag(x)) for prop with t0=" << 4 << std::endl;
    for(int t=0;t<Lt;t++){
      std::cout << t << " " << trace(C[t]) << std::endl;
    }
  }

  std::vector<RealD> Cts_t1 = momWallSourcePionCorrelator(p, p, 1, R_t1, R_t1);
  std::vector<RealD> Cts_t4 = momWallSourcePionCorrelator(p, p, 4, R_t4, R_t4);
